...
from .pact_functions import (AlmostSymmQuantFunc, PACTQuantize, TQTQuantize)
from .util import assert_param_valid, mse_bounds
...
class RequantShift(nn.Module):
    ...
    class MyRequantShift(torch.autograd.Function):
        ...
        def symbolic(g, x, mul, add, div, signed, n_levels_out, cmsis_requant):
            ...
            output = g.op("PACTOps::RequantShift",
                          x,
                          mul,
                          add,
                          div_t = div,
                          signed_t = signed,
                          n_levels_out_t = n_levels_out)
            return output
    ...
    def __init__(self,
                 ...
                 n_levels: int,
                 signed: bool = False,
                 D: torch.Tensor = torch.Tensor((2**16,)),
                 cmsis_requant = False,
                 requant_node = True):
    ...
    def forward(self, x):
        ...
        if self.requant_node:
            ...
            return self.MyRequantShift.forward(None, x, mul, add, self.div, self.signed, self.n_levels_out,
                                               self.cmsis_requant)
    ...
    class MyChannelwiseThreshold(torch.autograd.Function):
        ...
        def symbolic(g, x, thresh_lo, thresh_hi, signed_out):
            return g.op("PACTOps::ChannelwiseThreshold2d",
                        x,
                        thresh_lo_i = thresh_lo,
                        thresh_hi_i = thresh_hi,
...
class _PACTActivation(nn.Module):
    ...
    def __init__(
        ...
        if act_kind == 'relu6':
            self.clip_hi = torch.nn.Parameter(torch.Tensor((6.,)),
                                              requires_grad = (learn_clip and not symm) and not tqt)
        else:
            self.clip_hi = torch.nn.Parameter(torch.Tensor((1.,)),
                                              requires_grad = (learn_clip and not symm) and not tqt)
        ...
        if self.tqt:
            ...
            ), f"{self.__class__.__name__}: TQT quantization requires noisy=False, learn_clip=True, symm=True - you provided noisy={noisy}, learn_clip={learn_clip}, symm={symm}"
    ...
    def updateHistogram(self, stat):
        ...
        def rebinInt(histogram, factor):
            ...
            res = torch.nn.functional.conv1d(histogram.reshape(1, 1, -1),
                                             weight,
                                             bias = None,
                                             stride = factor,
                                             padding = 0)
            ...
            if self.signed:
                newHistogram[(self.num_bins // 2 - self.num_bins // (2 * factor)):(self.num_bins // 2 + self.num_bins //
                                                                                   (2 * factor))] = res.reshape(-1)
            else:
    ...
    def updateClipBounds(self):
        ...
        cdf = torch.nn.functional.conv1d(pdf.reshape(1, 1, -1),
                                         weight,
                                         bias = None,
                                         stride = 1,
                                         padding = (len(self.histogram) - 1)).reshape(-1)
    ...
    def forward(self, x):
        if not self.started:
            ...
            x_stat = torch.tensor(res, device = self.max.device,
                                  dtype = self.max.dtype) if not isinstance(res, torch.Tensor) else res
            ...
            if self.init_clip == 'percentile' and self.ready:
                ...
                with torch.no_grad():
                    if self.init_clip != 'mse':
                        ...
                        mse_min, mse_max = mse_bounds(x_stat, self.n_levels, self.signed, False, False, self.mse_iters,
                                                      self.symm, self.rounding)
                        ...
                        self.min[:] = new_min
                        self.max[:] = new_max
                    self.running_mean[:] = self.ema_beta * self.running_mean.item() + (1 -
                                                                                       self.ema_beta) * x_stat.mean()
                    self.running_var[:] = self.ema_beta * self.running_var.item() + (
                        1 - self.ema_beta) * x_stat.std() * x_stat.std()
            ...
            if self.tqt:
                ...
                result = TQTQuantize(x, eps, self.log_t, self.clip_lo, self.clip_hi, self.tqt_beta,
                                     self.tqt_running_grad_var, self.tqt_running_beta, self.tqt_clip_grad,
                                     self.rounding)
            else:
            ...
            x_stat = torch.tensor(result, device = self.max.device,
                                  dtype = self.max.dtype) if not isinstance(result, torch.Tensor) else result
            x_stat = x_stat[torch.isfinite(x_stat)]
...
class PACTIntegerConcat(torch.nn.Module):
    ...
    def __init__(self, num_args = 1, dim: int = 0, stack_flag: bool = False, signed: bool = True, **kwargs):
...
class PACTIntegerAdd(torch.nn.Module):
    ...
    def __getattribute__(self, name):
        if name == "n_levels":
            warnings.warn(
                "PACTIntegerAdd.n_levels was accessed - this member is deprecated; please use n_levels_in and n_levels_out!"
            )
...
class PACTConv2d(nn.Conv2d, _PACTLinOp):
    ...
    def __init__(self,
        ...
        if pm not in valid_padding_modes:
            assert pm in [
                'eps', 'neg_ones'
            ], f'PACTConv2d got invalid padding mode {pm} - expected one of {valid_padding_modes.union({"eps", "neg_ones"})}'
    ...
    def forward(self, x):
        ...
        if self.padding_mode != 'zeros':
            ...
            pad_val = 0.0 if (not self.started or not isinstance(x, QTensor) or x.eps
                              is None) else x.eps.item() * self.eps_pad_fac if self.padding_mode == 'eps' else -1.0
    ...
    def get_bias_q(self, eps_in):
        ...
        with torch.no_grad():
            b = PACTQuantize(self.bias,
                             self.get_eps_out(eps_in).flatten(),
                             -2**16 * self.get_eps_out(eps_in).flatten() * torch.ones_like(self.clip_lo.flatten()),
                             2**16 * self.get_eps_out(eps_in).flatten() * torch.ones_like(self.clip_hi.flatten()),
                             clip_gradient = self.clip_gradient,
                             floor = False)
        return b
...
class PACTConv1d(nn.Conv1d, _PACTLinOp):
    ...
    def __init__(self,
        ...
        if pm not in valid_padding_modes:
            assert pm in [
                'eps', 'neg_ones'
            ], f'PACTConv1d got invalid padding mode {pm} - expected one of {valid_padding_modes.union({"eps", "neg_ones"})}'
            # for Conv1d initializer, pretend we are doing zero padding...
            kwargs['padding_mode'] = 'zeros'
    ...
    def forward(self, x):
        ...
        if self.padding_mode != 'zeros':
            ...
            pad_val = 0.0 if (not self.started or not isinstance(x, QTensor)
                              or x.eps is None) else x.eps.item() * self.eps_pad_fac if mode == 'eps' else -1.0
            x = nn.functional.pad(x, self._reversed_padding_repeated_twice, mode = mode, value = pad_val)
            padding = 0
        else:
            padding = self.padding
...
class PACTCausalConv1d(PACTConv1d, _PACTLinOp):
    ...
    def __init__(self,
        ...
        if pm not in valid_padding_modes:
            assert pm in [
                'eps', 'neg_ones'
            ], f'PACTCausalConv1d got invalid padding mode {pm} - expected one of {valid_padding_modes.union({"eps", "neg_ones"})}'
        ...
        super(PACTCausalConv1d, self).__init__(in_channels,
                                               out_channels,
                                               kernel_size,
                                               n_levels,
                                               quantize,
    ...
    def from_causalconv1d(cls, c: CausalConv1d, **kwargs):
        ...
        pact_causalconv = cls(in_channels = c.in_channels,
                              out_channels = c.out_channels,
                              kernel_size = c.kernel_size,
                              stride = c.stride,
                              dilation = c.dilation,
...
class PACTIntegerHardswish(nn.Module):
    ...
    def forward(self, x):
        if self.export_node:
            return self.MyHardswish.apply(x, int(self.three.item()), int(self.six.item()),
                                          int(self.one_over_six.item()))
        else:
    ...
    def __init__(self,
        ...
        if twoStage:
            ...
            self.register_buffer(
                ...
                torch.Tensor(
                    torch.round(
                        PACTQuantize(weight, eps_bias, clip_lo, clip_hi, self.floor, self.clip_gradient, self.noisy) /
                        eps_bias)).detach())
            self.rqs1 = RequantShift(mul = torch.floor(D * eps_in / eps_adder),
                                     add = torch.Tensor((0.,)),
                                     signed = True,
                                     D = torch.Tensor((D,)),
                                     n_levels = n_levels,
            ...
            self.rqs2 = RequantShift(mul = torch.floor(D * eps_adder / eps_out),
                                     add = torch.Tensor((0.,)),
                                     signed = True,
                                     D = torch.Tensor((D,)),
                                     n_levels = n_levels,
        ...
        # Requantize in one step - Fewer operations, but the quantization error might be larger
            ...
            self.register_buffer(
                'weight',
                torch.round(
                    PACTQuantize(weight, eps_out / D, clip_lo, clip_hi, self.floor, self.clip_gradient, self.noisy) /
                    (eps_out / D)).detach())
            self.rq = RequantShift(mul = torch.floor(D * eps_in / eps_out),
                                   add = self.weight,
                                   signed = True,
                                   D = torch.Tensor((D,)),
                                   n_levels = n_levels,
...
class PACTIntegerExp(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.export_node:
            return self.MyExp.apply(x, self.log2.type_as(x), self.coeffA.type_as(x), self.coeffB.type_as(x),
                                    self.coeffC.type_as(x), self.n_levels.type_as(x), self.zero.type_as(x))
        else:
            return self.MyExp.forward(None, x, self.log2.type_as(x), self.coeffA.type_as(x), self.coeffB.type_as(x),
                                      self.coeffC.type_as(x), self.n_levels.type_as(x), self.zero.type_as(x))
...
class PACTITAPartialMax(_PACTEps):
    ...
    def __init__(self,
        ...
        assert self.max_estimation in [
            'statistical', 'global'
        ], f"[PACTITAPartialMax] Invalid max estimation method: {self.max_estimation}! Select either 'statistical' or 'global'!"
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            if self.act.started and self.max_estimation == 'statistical':
                # Use maximum gather by statistics
                x = x - torch.repeat_interleave(self.act.max, H * S * S).reshape(-1, H, S, S) + ((
                    (self.n_levels - 1) / 2)) * eps
        ...
        # Calculate the activation value
        ret = RQ(torch.repeat_interleave(exp_partial_sum_inverse, S).reshape(-1, H, S, S) / 2**shift, 1,
                 round = False) / (self.n_levels - 1)
        return ret
...
class PACTIntegerITAPartialMax(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.export_node:
            ...
            return self.MySoftmax.forward(None, x_rq, self.n_levels.type_as(x), int(self.group_width),
                                          int(self.eps_max_factor))
...
class PACTGELU(_PACTEps):
    ...
    def updateCoeffs(self, eps_in):
        ...
        with torch.no_grad():
            ...
            self.sqrttwo = RQ(
                torch.Tensor((math.sqrt(2.),)).type_as(self.sqrttwo),
                torch.Tensor((math.sqrt(2.),)).type_as(self.sqrttwo))
...
class PACTIntegerLayerNorm(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 maxval: float = 1.,
                 weight: torch.Tensor = torch.Tensor((1.,)),
                 bias: torch.Tensor = torch.Tensor((0.,)),
                 D = 2**24,
                 export_node = False,
                 **kwargs):
        super().__init__()
        ...
        if not torch.equal(weight, dummyOne) and not torch.equal(bias, dummyZero):
            ...
            self.register_buffer(
                ...
                torch.Tensor(
                    torch.round(
                        PACTQuantize(weight, eps_weights, clip_lo, clip_hi, self.floor, self.clip_gradient, self.noisy)
                        / eps_weights).detach()))
            self.register_buffer(
                "bias",
                torch.Tensor(
                    torch.round(
                        PACTQuantize(bias, eps_bias, clip_lo, clip_hi, self.floor, self.clip_gradient, self.noisy) /
                        eps_bias).detach()))
            self.register_buffer(
                "totScaler",
                torch.Tensor((torch.round(self.D * (n_levels // 2 - 1) / maxval * eps_weights),)).detach())
            ...
            self.register_buffer("totScaler",
                                 torch.Tensor((torch.round(self.D * (n_levels // 2 - 1) / maxval),)).detach())
            self.register_buffer("weight", self.totScaler)

    def forward(self, x):
        if self.export_node:
            return self.MyLayerNorm.apply(x, self.weight.type_as(x), self.bias.type_as(x), self.D.type_as(x),
                                          self.n_levels.type_as(x))
        else:
            return self.MyLayerNorm.forward(None, x, self.weight.type_as(x), self.bias.type_as(x), self.D.type_as(x),
                                            self.n_levels.type_as(x))
...
class PACTLayerNorm(_PACTEps, _PACTLinOp):
    ...
    def get_bias_q(self):
        ...
        with torch.no_grad():
            b = PACTQuantize(self.bias,
                             eps,
                             -2**16 * self.get_eps_out(eps_in).flatten() * torch.ones_like(self.clip_lo),
                             2**16 * self.get_eps_out(eps_in).flatten() * torch.ones_like(self.clip_hi),
                             clip_gradient = self.clip_gradient)
        return b
    ...
    def forward(self, x):
        ...
        assert torch.sum(
            torch.isnan(var)
        ) == 0, f"Variance has nan values in PACTLayerNorm, eta = {self.eta}, eps = {self.eps}, eps_in = {self.eps_in}, var = {var}"
        assert torch.prod(
            var > torch.zeros_like(var)
        ), f"Variance is zero or negative in PACTLayerNorm, eta = {self.eta}, eps = {self.eps}, eps_in = {self.eps_in}, var = {var}"
...
class PACTWrapLinearAttention(nn.Module):
    ...
    class LinearAttention(torch.autograd.Function):
        ...
        @staticmethod
        def forward(ctx, q, k, v, wq_weight, wq_bias, wk_weight, wk_bias, wv_weight, wv_bias, wo_weight, wo_bias,
                    wq_requant_mul, wq_requant_div, wk_requant_mul, wk_requant_div, wv_requant_mul, wv_requant_div,
                    preattn_requant_mul, preattn_requant_div, normalizer_requant_mul, normalizer_requant_div,
                    postattn_requant_mul, postattn_requant_div, wo_requant_mul, wo_requant_div, dim, heads, dim_head,
                    Delta, eps, eta, act_type, n_levels):
            return q

        @staticmethod
        @parse_args('v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 't', 't', 't', 't', 't', 't', 't', 't', 't',
                    't', 't', 't', 't', 't', 't', 't', 't', 'i', 'i', 'i', 'i', 't')
        def symbolic(g, q, k, v, wq_weight, wq_bias, wk_weight, wk_bias, wv_weight, wv_bias, wo_weight, wo_bias,
                     wq_requant_mul, wq_requant_div, wk_requant_mul, wk_requant_div, wv_requant_mul, wv_requant_div,
                     preattn_requant_mul, preattn_requant_div, normalizer_requant_mul, normalizer_requant_div,
                     postattn_requant_mul, postattn_requant_div, wo_requant_mul, wo_requant_div, dim, heads, dim_head,
                     Delta, eps, eta, act_type, n_levels):
            return g.op("PACTOps::LinearAttention",
                        ...
                        wo_weight,
                        wo_bias,
                        wq_requant_mul_t = wq_requant_mul,
                        wq_requant_div_t = wq_requant_div,
                        wk_requant_mul_t = wk_requant_mul,
                        wk_requant_div_t = wk_requant_div,
                        wv_requant_mul_t = wv_requant_mul,
                        wv_requant_div_t = wv_requant_div,
                        wo_requant_mul_t = wo_requant_mul,
                        wo_requant_div_t = wo_requant_div,
                        preattn_requant_mul_t = preattn_requant_mul,
                        preattn_requant_div_t = preattn_requant_div,
                        normalizer_requant_mul_t = normalizer_requant_mul,
                        normalizer_requant_div_t = normalizer_requant_div,
                        postattn_requant_mul_t = postattn_requant_mul,
                        postattn_requant_div_t = postattn_requant_div,
                        dim_t = dim,
                        heads_t = heads,
                        dim_head_t = dim_head,
                        Delta_i = Delta,
    ...
    def forward(self, q, k, v, **kwargs):
        if self.linearattention_node:
            return self.LinearAttention.apply(q, k, v, self.wq_weight.type_as(q), self.wq_bias.type_as(q),
                                              ...
                                              self.normalizer_requant_mul.type_as(q),
                                              self.normalizer_requant_div.type_as(q),
                                              self.postattn_requant_mul.type_as(q),
                                              self.postattn_requant_div.type_as(q), self.wo_requant_mul.type_as(q),
                                              self.wo_requant_div.type_as(q), self.dim.type_as(q),
                                              self.heads.type_as(q), self.dim_head.type_as(q), self.Delta, self.eps,
                                              self.eta, self.act_type, self.n_levels.type_as(q))
        else:
            return self.LinearAttention.forward(None, q, k, v, self.wq_weight.type_as(q), self.wq_bias.type_as(q),
                                                self.wk_weight.type_as(q), self.wk_bias.type_as(q),
                                                self.wv_weight.type_as(q), self.wv_bias.type_as(q),
                                                self.wo_weight.type_as(q), self.wo_bias.type_as(q),
                                                self.wq_requant_mul.type_as(q), self.wq_requant_div.type_as(q),
                                                self.wk_requant_mul.type_as(q), self.wk_requant_div.type_as(q),
                                                self.wv_requant_mul.type_as(q), self.wv_requant_div.type_as(q),
                                                self.preattn_requant_mul.type_as(q),
                                                self.preattn_requant_div.type_as(q),
                                                self.normalizer_requant_mul.type_as(q),
                                                self.normalizer_requant_div.type_as(q),
                                                self.postattn_requant_mul.type_as(q),
                                                self.postattn_requant_div.type_as(q), self.wo_requant_mul.type_as(q),
...
class PACTWrapCLCA(nn.Module):
    ...
    class CLCA(torch.autograd.Function):
        ...
        @staticmethod
        def forward(ctx, q, k, wq_weight, wq_bias, wkv_weight, wkv_bias, wo_weight, wo_bias, wq_requant_mul,
                    ...
                    postattn_requant_add, postattn_requant_div, wo_requant_mul, wo_requant_add, wo_requant_div, dim,
                    heads, dim_head, out_dim, Delta, eps, eta, act_type, n_levels):
            z = torch.empty(q.shape[0], out_dim, q.shape[2])
            return torch.ones_like(z)

        @staticmethod
        @parse_args('v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v',
                    'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 'v', 't', 't', 't', 't', 'i', 'i', 'i', 'i', 't')
        def symbolic(g, q, k, wq_weight, wq_bias, wkv_weight, wkv_bias, wo_weight, wo_bias, wq_requant_mul,
                     wq_requant_add, wq_requant_div, wk_requant_mul, wk_requant_add, wk_requant_div, wv_requant_mul,
                     wv_requant_add, wv_requant_div, kdiv_requant_mul, kdiv_requant_add, kdiv_requant_div,
                     preattn_requant_mul, preattn_requant_add, preattn_requant_div, postattn_requant_mul,
                     postattn_requant_add, postattn_requant_div, wo_requant_mul, wo_requant_add, wo_requant_div, dim,
    ...
    def forward(self, q, k, **kwargs):
        if self.linearattention_node:
            return self.CLCA.apply(q, k, self.wq_weight.type_as(q), self.wq_bias.type_as(q), self.wkv_weight.type_as(q),
                                   self.wkv_bias.type_as(q), self.wo_weight.type_as(q), self.wo_bias.type_as(q),
                                   self.wq_requant_mul.type_as(q), self.wq_requant_add.type_as(q),
                                   self.wq_requant_div.type_as(q), self.wk_requant_mul.type_as(q),
                                   self.wk_requant_add.type_as(q), self.wk_requant_div.type_as(q),
                                   self.wv_requant_mul.type_as(q), self.wv_requant_add.type_as(q),
                                   self.wv_requant_div.type_as(q), self.kdiv_requant_mul.type_as(q),
                                   self.kdiv_requant_add.type_as(q), self.kdiv_requant_div.type_as(q),
                                   self.preattn_requant_mul.type_as(q), self.preattn_requant_add.type_as(q),
                                   self.preattn_requant_div.type_as(q), self.postattn_requant_mul.type_as(q),
                                   self.postattn_requant_add.type_as(q), self.postattn_requant_div.type_as(q),
                                   self.wo_requant_mul.type_as(q), self.wo_requant_add.type_as(q),
                                   self.wo_requant_div.type_as(q), self.dim.type_as(q), self.heads.type_as(q),
                                   self.dim_head.type_as(q), self.out_dim.type_as(q), self.Delta, self.eps, self.eta,
                                   self.act_type, self.n_levels.type_as(q))
        else:
            return self.CLCA.forward(None, q, k, self.wq_weight.type_as(q), self.wq_bias.type_as(q),
                                     self.wkv_weight.type_as(q), self.wkv_bias.type_as(q), self.wo_weight.type_as(q),
                                     self.wo_bias.type_as(q), self.wq_requant_mul.type_as(q),
                                     self.wq_requant_add.type_as(q), self.wq_requant_div.type_as(q),
                                     self.wk_requant_mul.type_as(q), self.wk_requant_add.type_as(q),
                                     self.wk_requant_div.type_as(q), self.wv_requant_mul.type_as(q),
                                     self.wv_requant_add.type_as(q), self.wv_requant_div.type_as(q),
                                     self.kdiv_requant_mul.type_as(q), self.kdiv_requant_add.type_as(q),
                                     self.kdiv_requant_div.type_as(q), self.preattn_requant_mul.type_as(q),
                                     self.preattn_requant_add.type_as(q), self.preattn_requant_div.type_as(q),
                                     self.postattn_requant_mul.type_as(q), self.postattn_requant_add.type_as(q),
                                     self.postattn_requant_div.type_as(q), self.wo_requant_mul.type_as(q),
                                     self.wo_requant_add.type_as(q), self.wo_requant_div.type_as(q),
                                     self.dim.type_as(q), self.heads.type_as(q), self.dim_head.type_as(q),
                                     self.out_dim.type_as(q), self.Delta, self.eps, self.eta, self.act_type,
...
class PACTWrapMHSA(nn.Module):
    ...
    class MyMHSA(torch.autograd.Function):
        ...
        def forward(ctx, q, k, v, wq_weight, wq_bias, wk_weight, wk_bias, wv_weight, wv_bias, wo_weight, wo_bias,
                    wq_requant_mul, wq_requant_div, wk_requant_mul, wk_requant_div, wv_requant_mul, wv_requant_div,
                    preattn_requant_mul, preattn_requant_div, postattn_requant_mul, postattn_requant_div,
                    wo_requant_mul, wo_requant_div, dim, heads, dim_head, isoftmaxA, isoftmaxB, isoftmaxC, isoftmaxlog2,
                    n_levels, module):
            return module(q, k, v)
        ...
        def symbolic(g, q, k, v, wq_weight, wq_bias, wk_weight, wk_bias, wv_weight, wv_bias, wo_weight, wo_bias,
                     wq_requant_mul, wq_requant_div, wk_requant_mul, wk_requant_div, wv_requant_mul, wv_requant_div,
                     preattn_requant_mul, preattn_requant_div, postattn_requant_mul, postattn_requant_div,
                     wo_requant_mul, wo_requant_div, dim, heads, dim_head, isoftmaxA, isoftmaxB, isoftmaxC,
                     isoftmaxlog2, n_levels, module):
            ...
            return g.op("PACTOps::MultiHeadSelfAttention",
                        ...
                        heads_t = heads,
                        dim_head_t = dim_head,
                        isoftmaxA_t = isoftmaxA,
                        isoftmaxB_t = isoftmaxB,
                        isoftmaxC_t = isoftmaxC,
                        isoftmaxlog2_t = isoftmaxlog2,
                        n_levels_t = n_levels)

    def __init__(self, wq_weight, wq_bias, wq_requant_mul, wq_requant_div, wk_weight, wk_bias, wk_requant_mul,
                 wk_requant_div, wv_weight, wv_bias, wv_requant_mul, wv_requant_div, preattn_requant_mul,
                 preattn_requant_div, postattn_requant_mul, postattn_requant_div, wo_weight, wo_bias, wo_requant_mul,
                 wo_requant_div, dim, heads, dim_head, isoftmaxA, isoftmaxB, isoftmaxC, isoftmaxlog2, n_levels, module):
    ...
    def forward(self, q, k, v, **kwargs):
        return self.MyMHSA.apply(q, k, v, self.wq_weight.type_as(q), self.wq_bias.type_as(q), self.wk_weight.type_as(q),
                                 self.wk_bias.type_as(q), self.wv_weight.type_as(q), self.wv_bias.type_as(q),
                                 self.wo_weight.type_as(q), self.wo_bias.type_as(q), self.wq_requant_mul.type_as(q),
                                 self.wq_requant_div.type_as(q), self.wk_requant_mul.type_as(q),
                                 self.wk_requant_div.type_as(q), self.wv_requant_mul.type_as(q),
                                 self.wv_requant_div.type_as(q), self.preattn_requant_mul.type_as(q),
                                 self.preattn_requant_div.type_as(q), self.postattn_requant_mul.type_as(q),
                                 self.postattn_requant_div.type_as(q), self.wo_requant_mul.type_as(q),
                                 self.wo_requant_div.type_as(q), self.dim.type_as(q), self.heads.type_as(q),
                                 self.dim_head.type_as(q), self.isoftmaxA.type_as(q), self.isoftmaxB.type_as(q),
                                 self.isoftmaxC.type_as(q), self.isoftmaxlog2.type_as(q), self.n_levels.type_as(q),
                                 self.module)
...
class PACTTrueIntegerDiv(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.integer_node:
            ...
            return self.MyTrueIntegerDiv.apply(x, int(y), int(self.Delta.item()), int(self.eps.item()),
                                               int(self.eta.item()))
        else:
...
def SofterMax(A_requant, integerize = True):
    ...
    for i in range(n_heads):
        for j in range(x.shape[1]):
            for k in range(x.shape[2]):
                exp_result[i, j,
                           k] = (int(exp_result[i, j, k]) >> int(max_values[i, j, -1] - max_values[i, j, k])) / sum
