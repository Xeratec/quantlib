...
from quantlib.editing.lightweight.rules.filters import SubTypeFilter, TypeFilter
...
__all__ = [
    'PACTEpsController', 'PACTActController', 'PACTLinearController', 'PACTIntegerModulesController',
...
class PACTEpsController(Controller):
    ...
    def __init__(self, fx_model, modules, schedule, tracer, eps_pass, verbose = False):
        ...
        self.schedule = {
            int(k): v.lower() if isinstance(v, str) else [val.lower() for val in v] for k, v in schedule.items()
        }
...
class PACTActController(Controller):
    ...
    def __init__(self,
        ...
        assert all(
            isinstance(m, _PACTActivation) for m in modules), "Non-activation modules passed to PACTActController!"
                    ...
                    for m in self.modules:
                        for k, p in m.clipping_params.items():
                            try:
                                ...
                                symm = False
                            p.requires_grad = m.learn_clip and not (k == 'high' and symm) and not (k in ['low', 'high']
                                                                                                   and m.tqt)
                    self.frozen = False
    ...
    def reset_clip_bounds(self, m: Union[PACTUnsignedAct, PACTAsymmetricAct], method: str = None):
        ...
        if method in ['max', 'mse']:
            ...
            def cust_loss(x_hp, x_fq, bincenter, l1, l2):
                return torch.sum(
                    (torch.abs(x_hp - x_fq) *
                     torch.abs(bincenter))**2)  #* torch.cos(torch.abs(bincenter)/torch.max(bincenter) * pi/2)
            ...
            class Resampler(nn.Module):
                ...
                def forward(self, histogram, prevEdges, n_levels, max):
                    ...
                    innerSamples = torch.nn.functional.interpolate(oldHist.reshape(1, 1, -1),
                                                                   size = n_levels,
                    ...
                    innerSamples = torch.nn.functional.interpolate(innerSamples.reshape(1, 1, -1),
...
class PACTLinearController(Controller):
                ...
                if cmd == 'verbose_on':
                    ...
                    for m in self.modules:
                        for k, b in m.clipping_params.items():
                            ...
                            b.requires_grad = m.learn_clip and not (k == 'high' and
                                                                    m.symm_wts) and not (k in ['low', 'high'] and m.tqt)
                        m.frozen &= False
                    self.frozen = False
    ...
    def reset_clip_bounds(self,
                          m: Union[PACTConv2d, PACTConv1d, PACTLinear, PACTCausalConv1d],
                          method: str = None,
                          init: bool = False):
        if method is None:
            method = m.init_clip
        ...
        if init or not m.tqt:
            if method == 'max':
                ...
                min_val, max_val = mse_bounds(w, m.n_levels, True, m.quantize == 'per_channel', True, m.mse_iters,
                                              m.symm_wts)
                ...
                min_val, max_val = torch.where(alpha < 0, min_val_mm,
                                               min_val), torch.where(alpha < 0, max_val_mm, max_val)
...
class PACTIntegerModulesController(Controller):
    ...
    @staticmethod
    def get_modules(net: nn.Module):
        net_nodes_intmodules_intact = LightweightGraph.build_nodes_list(net,
                                                                        leaf_types = (PACTIntegerAdd,
                                                                                      PACTIntegerConcat))
