...
class FunctionalEquivalenceUnitGenerator(object):
    ...
    def __init__(self, factory: ANAModuleFactory):
        ...
        )  # it does not make sense to perform a functional comparison between two clones of the same thing (i.e., CPU and GPU versions)
    ...
    def get_test_unit(
        self, batch_size: BatchSize, input_size: InputSize, test_module: TestModule, noise_type: str, mi: float,
        sigma: float, strategy: str
    ) -> Tuple[Tuple[TensorGenerator, nn.Module, TensorGenerator], Tuple[TensorGenerator, nn.Module, TensorGenerator]]:
        ...
        if (test_module == TestModule.ACTIVATIONLINEAR) or (test_module == TestModule.LINEAR):
            ...
            x_gen_cpu = LinearTensorGenerator(self._cpu_device, batch_size, n_channels, Kind.LINSPACE,
                                              self._factory.range)
            x_gen_gpu = LinearTensorGenerator(self._gpu_device, batch_size, n_channels, Kind.LINSPACE,
                                              self._factory.range)
            ...
            x_gen_cpu = Conv2dTensorGenerator(self._cpu_device, batch_size, n_channels, spatial_size, Kind.LINSPACE,
                                              self._factory.range)
            x_gen_gpu = Conv2dTensorGenerator(self._gpu_device, batch_size, n_channels, spatial_size, Kind.LINSPACE,
                                              self._factory.range)
        else:
            raise ValueError
        ...
        if (test_module == TestModule.ACTIVATIONLINEAR) or (test_module == TestModule.ACTIVATIONCONV2D):
            module_cpu, output_size_cpu = self._factory.get_module('activation',
                                                                   True,
                                                                   self._cpu_device,
                                                                   x_gen_cpu.size,
                                                                   ...
                                                                   sigma = sigma,
                                                                   strategy = strategy)
            module_gpu, output_size_gpu = self._factory.get_module('activation',
                                                                   True,
                                                                   self._gpu_device,
                                                                   x_gen_gpu.size,
                                                                   noise_type = noise_type,
            ...
            module_cpu, output_size_cpu = self._factory.get_module('linear',
                                                                   True,
                                                                   self._cpu_device,
                                                                   x_gen_cpu.size,
                                                                   noise_type = noise_type,
                                                                   ...
                                                                   sigma = sigma,
                                                                   strategy = strategy)
            module_gpu, output_size_gpu = self._factory.get_module('linear',
                                                                   True,
                                                                   self._gpu_device,
                                                                   x_gen_gpu.size,
                                                                   noise_type = noise_type,
            ...
            module_cpu, output_size_cpu = self._factory.get_module('conv2d',
                                                                   True,
                                                                   self._cpu_device,
                                                                   x_gen_cpu.size,
                                                                   noise_type = noise_type,
                                                                   ...
                                                                   sigma = sigma,
                                                                   strategy = strategy)
            module_gpu, output_size_gpu = self._factory.get_module('conv2d',
                                                                   True,
                                                                   self._gpu_device,
                                                                   x_gen_gpu.size,
                                                                   noise_type = noise_type,
...
class ProfilingUnitGenerator(object):
    ...
    def __init__(self, factory: ModuleFactory, device: torch.device):

        self._factory = factory
        self._device = device

    def get_test_unit(self, batch_size: BatchSize, input_size: InputSize, test_module: TestModule,
                      **kwargs) -> Tuple[TensorGenerator, nn.Module, TensorGenerator]:

        # create input generator
        batch_size = batch_size.value
        if (test_module == TestModule.ACTIVATIONLINEAR) or (test_module
                                                            == TestModule.LINEAR) or (test_module
                                                                                      == TestModule.LINEARNETWORK):
            input_size_2_linear_size = {
                InputSize.SMALL: LinearSize.SMALL.value,
                InputSize.NORMAL: LinearSize.NORMAL.value,
                InputSize.LARGE: LinearSize.LARGE.value
        ...
        elif (test_module == TestModule.ACTIVATIONCONV2D) or (test_module
                                                              == TestModule.CONV2D) or (test_module
                                                                                        == TestModule.CONV2DNETWORK):
            input_size_2_conv2d_size = {
                InputSize.SMALL: (Conv2dChannels.SMALL.value, Conv2dSpatialSize.SMALL.value),
                InputSize.NORMAL: (Conv2dChannels.NORMAL.value, Conv2dSpatialSize.NORMAL.value),
                InputSize.LARGE: (Conv2dChannels.LARGE.value, Conv2dSpatialSize.LARGE.value)
