...
class TensorGenerator(object):
    ...
    def __init__(self, device: torch.device, kind: Kind, range_: Union[Range, None] = None):
    ...
    def __next__(self) -> torch.Tensor:
        ...
        if self._kind == Kind.ZEROS:
            ...
            x = torch.linspace(self._range.start, self._range.end, n).reshape(
                *self.size)  # x.view(-1) returns the linearly spaced values in a one-dimensional array
