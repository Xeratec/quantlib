...
class ModuleFactory(object):
    ...
    def get_module(self, class_name: str, training: bool, device: torch.device,
                   input_size: Union[Tuple[int, int], Tuple[int, int, int, int]],
                   **kwargs) -> [nn.Module, Union[Tuple[int, int], Tuple[int, int, int, int]]]:

        raise NotImplementedError

    def get_network(self, linear_or_conv2d: str, training: bool, device: torch.device,
                    input_size: Union[Tuple[int, int], Tuple[int, int, int, int]],
...
class FloatingPointModuleFactory(ModuleFactory):
    ...
    def get_module(self, class_name: str, training: bool, device: torch.device, input_size: Tuple,
...
class ANAModuleFactory(ModuleFactory):
    ...
    def get_module(self,
        ...
        if class_name == 'activation':
            ...
            module = class_(self._quantizer_spec, noise_type, strategy, in_features = N, out_features = N, bias = False)
            output_size = (B, N)
            ...
            module = class_(self._quantizer_spec,
                            noise_type,
                            strategy,
                            in_channels = Nin,
                            out_channels = Nout,
...
def create_quantizer_spec(nbits: int, signed: bool, balanced: bool, eps: float):

    quantizer_spec = {'nbits': nbits, 'signed': signed, 'balanced': balanced, 'eps': eps}
