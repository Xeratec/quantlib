...
def compare_tensors(t1: torch.Tensor, t2: torch.Tensor, tolerance: float) -> bool:
    ...
    if torch.all(equivalent):
        ...
        for coords in torch.logical_not(equivalent).nonzero():
            ...
            print("Difference at position {}: t1[{}] = {}, t2[{}] = {}.".format(coords, coords, t1[coords], coords,
                                                                                t2[coords]))
...
def distribution2d(x_all: torch.Tensor, y_all: torch.Tensor, quant_levels: torch.Tensor, eps: torch.Tensor) -> None:
    ...
    assert torch.all(x == torch.sort(
        x)[0])  # I assume that the inputs are generated by a `TensorGenerator` with `self._kind == Kind.LINSPACE`

    x_edges = torch.hstack([x, x.max() + torch.max(x[1:] - x[:-1])])
    y_edges = torch.hstack([
        torch.min(quant_levels) - eps / 2, (quant_levels[1:] + quant_levels[:-1]) / 2,
