...
class ANAModule(nn.Module):
    ...
    def __init__(self, quantizer_spec: Dict, noise_type: str, strategy: str):
        ...
        ANAModule.setup_noise(
            self, NoiseType[noise_type.upper()].value, ForwardComputationStrategy[strategy.upper()].value
    ...
    def setup_noise(anamod: nn.Module, noise_class, strategy: int):
        ...
        anamod.register_parameter('strategy',
                                  nn.Parameter(torch.Tensor([strategy]).to(torch.int32), requires_grad = False))
...
class ANAActivation(ANAModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_out = self.ana_op(x, self.quant_levels, self.thresholds, self.mi, self.sigma, self.strategy, self.training)
...
class ANALinear(ANAModule):
    ...
    def weight_maybe_quant(self) -> torch.Tensor:
        ...
        weight = self.ana_op(weight, self.quant_levels, self.thresholds, self.mi, self.sigma, self.strategy,
                             self.training)
        return weight
...
class _ANAConvNd(ANAModule):
    ...
    def __init__(self, quantizer_spec: Dict, noise_type: str, strategy: str, in_channels: int, out_channels: int,
                 kernel_size: int, stride: int, padding: int, dilation: int, transposed: bool, output_padding,
...
class ANAConv1d(_ANAConvNd):
    ...
    def __init__(self,
        ...
        super(ANAConv1d, self).__init__(quantizer_spec, noise_type, strategy, in_channels, out_channels, kernel_size,
                                        stride, padding, dilation, False, _single(0), groups, bias)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return F.conv1d(input, self.weight_maybe_quant * self.eps, self.bias, self.stride, self.padding, self.dilation,
                        self.groups)
...
class ANAConv2d(_ANAConvNd):
    ...
    def __init__(self,
        ...
        super(ANAConv2d, self).__init__(quantizer_spec, noise_type, strategy, in_channels, out_channels, kernel_size,
                                        stride, padding, dilation, False, _pair(0), groups, bias)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return F.conv2d(input, self.weight_maybe_quant * self.eps, self.bias, self.stride, self.padding, self.dilation,
                        self.groups)
...
class ANAConv3d(_ANAConvNd):
    ...
    def __init__(self,
        ...
        super(ANAConv3d, self).__init__(quantizer_spec, noise_type, strategy, in_channels, out_channels, kernel_size,
                                        stride, padding, dilation, False, _triple(0), groups, bias)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return F.conv3d(input, self.weight_maybe_quant * self.eps, self.bias, self.stride, self.padding, self.dilation,
                        self.groups)
