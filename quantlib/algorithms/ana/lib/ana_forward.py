...
def forward_random(pmf: torch.Tensor, q: torch.Tensor):
    ...
    idxs = torch.vstack(
        [(1.0 - torch.sum(sc, dim = 0)).unsqueeze(0), sc]
    )  # the reduction performed by `torch.sum` must be compensated by the "inflation" provided by `unsqueeze`, so that the tensors can be concatenated
