...
__all__ = ['LayerIdentifier', 'extract_unique_layers', 'export_bench_spec', 'ident_of_layer']
...
class LayerIdentifier:
    ...
    def spec_dict(self, in_bits: int, out_bits: int, wt_bits: int):
        chin = max(int(np.ceil(self.ch_in / (8 // in_bits)) * 8 // in_bits),
                   int(np.ceil(self.ch_in / (8 // wt_bits)) * 8 // wt_bits))
    ...
    def from_series(cls, df: pd.Series):
        ...
        return cls(
            is_lin, *[
                df[k].item() for k in
                ['kernel_size', 'input_size', 'output_size', 'DW', 'groups', 'padding', 'stride', 'chin', 'chout']
            ])
...
def ident_of_conv2d(m: nn.Module, in_size: torch.Size, out_size: torch.Size):
    ...
    assert m.kernel_size[0] == m.kernel_size[
        1], f"ident_of_conv2d got module with non-quadratic kernel_size {m.kernel_size} - this is currently not supported"
    assert in_size[2] == in_size[
        3], f"ident_of_conv2d got non-quadratic input shape {in_size[2:]} - this is currently not supported"
    assert out_size[2] == out_size[
        3], f"ident_of_conv2d got non-quadratic output shape {out_size[2:]} - this is currently not supported"
    return LayerIdentifier(False, m.kernel_size[0], in_size[2], out_size[2], dw, m.groups, int(m.padding[0]),
                           int(m.stride[0]), m.in_channels, m.out_channels)
...
def extract_unique_layers(network: fx.GraphModule,
                          *shapes_in,
...
def build_ident_dict(network: fx.GraphModule,
                     *shapes_in,
                     dtype_in: torch.dtype = torch.float32,
                     tracer: callable = None):
    from quantlib.editing.fx.passes import ShapePropPass
...
def export_bench_spec(layers: set, export_folder: str, out_fn: str, in_bits: list, wt_bits: list):
    out_df = pd.DataFrame(columns = [
        'groups', 'DW', 'kernel_size', 'chin', 'chout', 'input_size', 'output_size', 'IN_BITS', 'OUT_BITS', 'W_BITS'
    ])
...
def import_bench_results(filename: str, latency_key: str = 'latency'):
    ...
    result_df['identifier'] = list(
        zip(*tuple(result_df[k] for k in
                   ['groups', 'DW', 'kernel_size', 'chin', 'chout', 'input_size', 'output_size', 'padding', 'stride'])))
...
def build_net_latency_dict(net: nn.Module,
    ...
    for k in kl:
        ...
        if act_name == 'input':
            ...
            try:
                ...
                ld[k]['props']['latency'] = {
                    (ap, wp): lat for (ap, wp), lat in ld[k]['props']['latency'].items() if ap == act_prec
                }
...
def add_freebie_cfg(pd: dict):
    ...
    for k in kl:
        ...
        better_lats = {
            (ap, wp): v
            for (ap, wp), v in cur_lat_dict.items()
            if ap >= cur_precs[0] and wp >= cur_precs[1] and v < cur_lat
        }
        if len(better_lats):
