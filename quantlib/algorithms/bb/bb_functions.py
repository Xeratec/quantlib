...
def BBQuantize(inp: torch.Tensor,
               phi: torch.Tensor,
               hc_lo: float,
               hc_hi: float,
               T: float,
    ...
    inp_q = torch.stack([
        PACTQuantize(inp,
                     ...
                     c_h,
                     floor = False,
                     clip_gradient = torch.tensor(True, device = inp.device),
                     noisy = False) for e, c_l, c_h in zip(eps, clip_lo, clip_hi)
    ])
...
def BBQuantizeTestTime(inp: torch.Tensor, phi: torch.Tensor, hc_lo: float, hc_hi: float, T: float,
                       clip_lo: torch.Tensor, clip_hi: torch.Tensor, precs: list, symm: bool):

    gates = torch.cat(
        (torch.tensor([True], device = inp.device), bb_cdf(torch.zeros([], device = inp.device), phi, hc_lo, hc_hi, T)
         < 0.34))
