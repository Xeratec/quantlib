...
def BB_attach_gates_individual_best_latency(layer_dict: dict, gate_init: float = 2., input_prec: int = 8):
    ...
    if len(layer_dict) == 2:
        ...
        assert len(
            layer_dict
        ) == 1, f"BB_attach_gates_individual_best_latency expects to get 1 or 2 layers - got {len(layer_dict)}"
        act_layer = None
...
def BB_attach_gates_shared(layer_dict: dict, gate_init: float = 2.):
    ...
    assert all(p == n_precs[0]
               for p in n_precs), "Unequal number of precisions in BB_attach_gates_shared... this does not make sense!"
    assert all(
        p == precs[0] for p in precs), "Unequal precisions in BB_attach_gates_shared... this does not make sense!"
...
class BBGateController:
    ...
    def __init__(self, layer: nn.Module, loss_scale: Optional[float] = None, gate_init: float = 2.):
        ...
        assert isinstance(
            layer,
            (BBConv2d, BBLinear, BBAct)), "Layer of BBGateController must be BBAct, BBLinear, BBConv1d or BBConv2d!"
        ...
        assert "bb_gates" not in dict(
            layer.named_parameters()), "Expected linear layer to not have 'bb_gates' parameter!"
        gates = nn.Parameter(torch.full((n_precs - 1,), gate_init))
        layer.bb_gates = gates
    ...
    def __init__(self, layer_dict: dict, regularizers: list, gate_init_fun: callable, gate_init_kwargs):
        ...
        for n, l in self.layers.items():
            try:
                ...
                print(
                    f"BBMultiLayerController: Did not register BB gate controller for layer {n} of type {type(l[0])} as it does not seem to be a BB layer..."
                )
...
class BBBOPComplexityRegularizer(BBRegularizer):
    ...
    def register_layers(self, layers: dict):
        ...
        self.op_layer_name, op_layer_and_props = [
            (k, l) for k, l in layers.items() if isinstance(l[0], tuple(_BB_LINOPS))
        ][0]
        ...
        act_layer = None
        if len(layers) != 1:
            self.act_layer_name, act_layer_and_props = [(k, v) for k, v in layers.items() if v[0] is not self.op_layer
                                                       ][0]
...
class BBLatencyRegularizer(BBRegularizer):
    ...
    def loss_term(self):
        ...
        move(torch.tensor(self.op_layer.precs))
...
class BBSplitLatencyRegularizer(BBLatencyRegularizer):
    ...
    def __init__(self, input_prec: int, mode: str):
        self.input_prec = input_prec
        mode = mode.lower()
        assert mode in ["decompose", "marginalize"
                       ], f"BBSplitLatencyRegularizer expected 'mode' to be 'decompose' or 'marginalize - got {mode}"
        self.mode = mode
    ...
    def register_layers(self, layers: dict):
        ...
        if self.act_layer is not None:
            if self.mode == "decompose":
                ...
                contribution_w = [
                    torch.zeros(self.scales.shape[0], self.scales.shape[1]) for _ in range(self.scales.shape[1])
                ]
...
class BBExportController(Controller):
    ...
    def __init__(self,
                 ...
                 net: Optional[fx.GraphModule] = None):
            ...
            if isinstance(n.module, tuple(_BB_LINOPS)) and self.net is not None:
                if len(n.module.gate_ctrls) == 1 and isinstance(n.module.gate_ctrls[0], BBGateController):
                    ...
                    if gate_ctrl.linked_layer is not None:
                        ...
                        linked_act = [
                            n_.module
                            for n_ in self.nodes_list
                            if self.fix_node_name(n_.name) == n.module.gate_ctrl.linked_layer
                        ][0]
                    ...
                    if all(isinstance(r, BBLatencyRegularizer) for r in n.module.gate_ctrls[0].regularizers):
                        ...
                        better_lats = {
                            ...
                            if ap >= in_prec and wp >= module_prec and v < cur_lat
                        }
                        if len(better_lats):
