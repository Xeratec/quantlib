...
__all__ = ["_BB_CLASSES", "_BB_LINOPS", "BBAct", "BBConv2d", "BBLinear", "BBIntegerAdd"]
...
class BBConv2d(PACTConv2d):
    ...
    def weight_q(self):
        if self.training:
            ...
            return BBQuantizeTestTime(self.weight, self.bb_gates, self.hc_lo, self.hc_hi, self.hc_T, self.clip_lo,
                                      self.clip_hi, self.precs, self.symm_wts)
    ...
    def get_n_levels(self):
        gates = torch.cat((torch.tensor([True], device = self.bb_gates.device),
                           bb_cdf(torch.zeros([], device = self.bb_gates.device), self.bb_gates, self.hc_lo, self.hc_hi,
                                  self.hc_T) < 0.34))
...
class BBLinear(PACTLinear):
    ...
    def __init__(self, in_features, out_features, precs, hc_stretch, hc_T, quantize, init_clip, **kwargs):
...
class BBAct(_PACTActivation):
    ...
    def __init__(self, precs, hc_stretch, hc_T, init_clip, learn_clip, act_kind, signed, leaky = 0.1, nb_std = 3):
    ...
    def forward(self, x):
        if not self.started:
            ...
            return BBQuantizeTestTime(x, self.bb_gates, self.hc_lo, self.hc_hi, self.hc_T, self.clip_lo, self.clip_hi,
                                      self.precs, self.signed)
...
_BB_CLASSES = [BBAct, BBConv2d, BBLinear]

_BB_LINOPS = [BBConv2d, BBLinear]


class BBIntegerAdd(PACTIntegerAdd):

    def __init__(self, num_args = 1, signed: Union[bool, list] = True, pact_kwargs: dict = {}, bb_kwargs: dict = {}):
        ...
        assert len(
            signed) == num_args + 1, f"BBIntegerAdd expected {num_args+1} elements in 'signed', got {len(signed)}"
