...
__all__ = [
    'FxPass', 'SequentialPass', 'ModifyMatchedModulesPass', 'ModifyMatchesPass', 'ModifySequentialPatternPass',
    'FindSequentialPatternsPass', 'ReplaceMatchWithModulePass', 'ReplaceSequentialPatternPass',
    'ReplaceSingleInputPatternPass', 'ModularizeNodePass', 'ModularizePass', 'ConstShapePass'
]
...
class FindSequentialPatternsPass(ModifySequentialPatternPass):
    ...
    def __init__(self, pattern: callable, trace: callable, name: str, include_objects: bool = False):
        ...
        super(FindSequentialPatternsPass, self).__init__(pattern,
                                                         trace,
                                                         self.register_match,
                                                         name,
                                                         modify_matches = True)
    ...
    def run_pass(self, gm: fx.GraphModule):
        ...
        if self.module is not None:
            ...
            try:
                with gm.graph.inserting_after(first_matched_node.all_input_nodes[0]):
                    # TODO: The bug's here
                    new_node = gm.graph.call_module(target,
...
class ReplacePartialSingleInputMatchWithModulePass(FxPass):
    ...
    def run_pass(self, gm: fx.GraphModule):
        matched_nodes = list(reversed([v for v in self.match.nodes_map.values()
                                      ][1:]))  #get_ordered_active_nodes(self.match)
            ...
            if self.op == 'call_module' and node.op == self.op:
                ...
                for _target in self.target:
                    ...
                    if targetClass == ownClass:
                        ...
                        passes.append(
                            ModularizeNodePass(node, f"_QL_{self.name.upper()}_MODULARIZED{replaced_fn}_{i}",
                                               self.replacement_fn))
                        i += 1
