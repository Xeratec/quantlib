...
from quantlib.editing.fx.passes import FxPass, ShapePropPass, CountMACsPass, MemoryUsagePass, CollectPropertiesPass
from quantlib.algorithms.pact import PACTIntegerAdd
from .bb_util import find_layer_sets, partition_dict

from quantlib.algorithms.bb import BBAct, BBGateController, BBBOPComplexityRegularizer, BBLatencyRegularizer, BBSplitLatencyRegularizer, BBMultiLayerController, BB_attach_gates_individual, BB_attach_gates_shared, BB_attach_gates_individual_best_latency
...
_ALL_DEF_ARGS = {"precs": [2, 4, 8], "hc_stretch": 1.2, "hc_T": 0.5, "init_clip": "max"}

_CONV_DEF_ARGS = _ALL_DEF_ARGS.copy()
_CONV_DEF_ARGS.update({"in_channels": 1, "out_channels": 1, "kernel_size": 1, "quantize": "per_channel"})

_LIN_DEF_ARGS = _ALL_DEF_ARGS.copy()
_LIN_DEF_ARGS.update({"in_features": 1, "out_features": 1, "quantize": "per_channel"})

_ACT_DEF_ARGS = _ALL_DEF_ARGS.copy()
_ACT_DEF_ARGS.update({"act_kind": "relu", "signed": False, "learn_clip": False})
...
class BBAttachControllersPass(FxPass):
    ...
    # a linear operator
    _PASSTHRU_LAYERS = (nn.AdaptiveAvgPool1d, nn.AdaptiveAvgPool2d, nn.AvgPool1d, nn.AvgPool2d, nn.AdaptiveMaxPool1d,
                        nn.AdaptiveMaxPool2d, nn.MaxPool1d, nn.MaxPool2d, nn.Flatten, nn.Dropout)
    ...
    def find_prev_act(self, gm: fx.GraphModule, node: fx.Node):
        if node.op in ["call_method", "call_function"]:
            print(
                f"BBAttachControllersPass: find_prev_act ignoring node {node.op}({node.target}) and continuing to node {node.all_input_nodes[0]}! If this is not correct, go and fix the code :^)"
            )
        ...
        for node in gm.graph.nodes:
            if node.op == "call_module" and isinstance(module_of_node(gm, node), tuple(_BB_LINOPS)):
                ...
                if "bb_gates" not in dict(module.named_parameters()):
                    ...
                    BBGateController(module, scale_factor, self.gate_init)
                    assert len(module.gate_ctrls
                              ) == 1, "OOPS, only single gate controller per layer supported by BBAttachControllersPass"
                    ...
                    if maybe_act:
                        ...
                        if isinstance(am, BBAct):  #`am` may be a regular PACTAct...
                            if len(am.gate_ctrls):
                                ...
                                BBGateController(am, scale_factor, self.gate_init)
        return gm
        ...
        for layer_name, node, module in named_module_nodes(gm):
            ...
            if ident is not None:
                try:
                    ...
                    print(
                        f"Warning: Layer {layer_name} has identifier, but was not found in latency spec file {self.latency_spec_file}... Something is about to break!"
                    )
                    prec_dict = None
...
class BBActConvControllerInitPass(FxPass):
    ...
    def __init__(self,
                 shape_in: Union[Tuple[int], List[int], torch.Size],
                 gate_init: float = 2.,
                 input_prec: int = 8,
                 ...
                 init_best_latency_gates: bool = False,
                 split: str = None):
        super(BBActConvControllerInitPass, self).__init__()
        assert target in [
            "bops", "latency"
        ], f"BBActConvControllerInitPass expected parameter 'target' with value 'bops' or 'latency', got '{target}'"
        if target == "latency":
    ...
    def run_pass(self, gm: fx.GraphModule):
        ...
        for pd in prop_dicts_partitioned:
            ...
            BBMultiLayerController(pd, [t_reg], self.attach_gate_fn, gate_init_kwargs)
