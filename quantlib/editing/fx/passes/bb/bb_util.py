...
__all__ = ["BB_OPS", "BBTracer", "BB_symbolic_trace", "find_prev_act", "find_layer_sets", "partition_dict"]

BB_OPS = set([BBAct, BBConv2d, BBLinear])
...
def find_prev_act(gm: fx.GraphModule, node: fx.Node):
    if node.op in ["call_method", "call_function"]:
        print(
            f"find_prev_act ignoring node {node.op}({node.target}) and continuing to node {node.all_input_nodes[0]}! If this is not correct, go and fix the code :^)"
        )
