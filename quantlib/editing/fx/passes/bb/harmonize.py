...
from .. import SequentialPass, InsertModuleBetweenModulesPass, AnnotateEpsPass
from ...util import module_of_node

from .bb_util import BB_symbolic_trace
from .. import SequentialPass, InsertModuleBetweenModulesPass, RetracePass
...
class BBAddTreeReplacementPass(OpTreeReplacementPass):
    ...
    def __init__(self,
                 infer_sign: bool = False,
                 infer_outact: bool = False,
                 pact_kwargs: dict = {},
                 bb_kwargs: dict = {},
                 **kwargs):
        ...
        self.kwargs = kwargs
        super(BBAddTreeReplacementPass, self).__init__(node_specs = self.add_node_specs,
    ...
    def add_replacement_fn(self, gm: fx.GraphModule, tree: OpTree):
        ...
        if self.infer_outact:
            if len(tree.users) == 1 and isinstance(module_of_node(gm, tree.users[0]),
                                                   (PACTUnsignedAct, PACTAsymmetricAct, BBAct)):
        ...
        return BBIntegerAdd(num_args = len(tree.args),
                            pact_kwargs = pact_kwargs_to_pass,
                            bb_kwargs = self.bb_kwargs,
                            **kwargs_to_pass)
