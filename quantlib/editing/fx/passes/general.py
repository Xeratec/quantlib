...
__all__ = [
    'MergeConvBNPass', 'ModularizeActivationsPass', 'RetracePass', 'InsertModuleAfterNodePass',
    'InsertModuleBetweenNodesPass', 'InsertModuleBetweenModulesPass', 'ShapePropPass', 'CountMACsPass',
    'MemoryUsagePass', 'CollectPropertiesPass', '_MAC_CNT_FNS'
]
...
_PROPERTY_TRANSL_FNS = {'tensor_meta': lambda v: {'out_shape': v.shape}}
...
_PROP_DICT_EXTENSION_FNS = {PACTIntegerAdd: intadd_outact_dict_update, BBIntegerAdd: intadd_outact_dict_update}
...
class MergeConvBNPass(SequentialPass):
    ...
    def __init__(self, trace: callable = fx.symbolic_trace):
        passes = []
        for conv, bn, dim in [(nn.Conv1d, nn.BatchNorm1d, "1D"), (nn.Conv2d, nn.BatchNorm2d, "2D"),
...
class ModularizeActivationsPass(ModularizePass):
    ...
    inplace_act_functions = (F.threshold_, F.relu_, F.hardtanh_, F.elu_, F.leaky_relu_, F.rrelu_)
    ...
    def __init__(self):
        super(ModularizeActivationsPass, self).__init__(op = 'call_function',
                                                        target = tuple(k for k in self.act_function_to_module.keys()),
                                                        replacement_fn = self.act_node_to_module,
            ...
            if node.op == 'call_module':
                ...
                if isinstance(m, self.modules_before):
                    ...
                    if len(insert_before_users) >= 1 and (
                        (len(insert_before_users) == len(node.users) and self.combine == 'conservative')
                        ...
                        for u, user_mod in insert_before_users:
                            ...
                            if new_module is not None:
                                passes.append(
                                    InsertModuleBetweenNodesPass(node, u, new_module, f"{self.name.upper()}_{idx}"))
                                idx += 1
