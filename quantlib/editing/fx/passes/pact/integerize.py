...
def integerize_softmax_fun(gm: fx.GraphModule,
                           match: Match,
                           mode: Literal["I-BERT", "ITA", 'ITA-Partial'] = "I-BERT",
                           D = 2**12,
                           export_node = False):
    ...
    if mode == 'I-BERT':
        ...
        new_softmax = PACTIntegerITAMax(max_value = module.act.max,
                                        n_levels = module.n_levels,
                                        eps_in = eps_in,
                                        D = D,
                                        export_node = export_node)
    elif mode == 'ITA-Partial':
        new_softmax = PACTIntegerITAPartialMax(max_value = module.act.max,
                                               n_levels = module.n_levels,
                                               eps_in = eps_in,
                                               eps_max_factor = module.eps_max_factor,
                                               max_estimation = module.max_estimation,
                                               D = D,
                                               export_node = export_node)
    else:
...
def integerize_hardswish_fun(gm: fx.GraphModule, match: Match, D = 2**14, export_node = False):
    ...
    eps_in = extract_eps(lin_node.meta['quant'].eps_in)
    assert isinstance(
        module, PACTHardswish), f"integerize_hardswish_fun got bad match - expected PACTHardswish, got {type(module)}"
...
class IntegerizeConstWrapPass(ModularizePass):
    ...
    def constwrap_replacement_fn(node):
        dict(node.graph._owning_module.named_modules())[node.target]
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(op = 'call_module',
                         target = tuple(target),
                         replacement_fn = self.constwrap_replacement_fn,
                         name = "CONSTWRAP_REPLACEMENT_PASS")
...
class IntegerizeTrueDivPass(ModularizePass):
    ...
    def truediv_replacement_fn(node, integer_node = False):
        ...
        if module.stable:
            return (PACTTrueIntegerDiv(module.Delta,
                                       eps = module.get_eps_div(),
                                       eta = module.eta,
                                       integer_node = integer_node), node.args, node.kwargs)
        else:
            return (PACTTrueIntegerDiv(module.Delta,
                                       eps = 0 * module.get_eps_div(),
                                       eta = module.eta,
                                       integer_node = integer_node), node.args, node.kwargs)
    ...
    def __init__(self, Delta = 2**14, export_div_node = False, **kwargs):
        ...
        super().__init__(op = 'call_module',
                         target = tuple(target),
                         replacement_fn = partial(self.truediv_replacement_fn, integer_node = export_div_node),
                         name = "TRUEDIV_REPLACEMENT_PASS")
...
def integerize_layernorm_fun(gm: fx.GraphModule, match: Match, affine = True, D = 2**12, export_node = False):
    ...
    assert isinstance(
        layernorm,
        PACTLayerNorm), f"integerize_layernorm_fun got bad match - expected PACTLayerNorm, got {type(module)}"
    ...
    if affine:
        ...
        new_layernorm = PACTIntegerLayerNorm(n_levels = requant.n_levels,
                                             eps_in = eps_in,
                                             maxval = maxval,
                                             weight = new_weight,
                                             bias = new_bias,
...
class IntegerizeLayerNormPass(SequentialPass):
    ...
    def __init__(self,
                 affine = True,
                 D = 2**12,
                 export_layernorm_node = False,
                 symbolic_trace: callable = PACT_symbolic_trace,
                 **kwargs):
        passes = []
        pattern = nn.Sequential(PACTLayerNorm(), PACTAsymmetricAct(256, 'max', True, 'relu'))
        passes.append(
            ReplaceSequentialPatternPass(
                pattern, symbolic_trace,
                partial(integerize_layernorm_fun, affine = affine, D = D, export_node = export_layernorm_node),
                f'_INTEGER_LAYERNORM_PASS'))
...
def integerize_rmsnorm_fun(gm: fx.GraphModule, match: Match, affine = True, D = 2**12, export_node = False):
    ...
    assert isinstance(rmsnorm,
                      PACTRMSNorm), f"integerize_rmsnorm_fun got bad match - expected PACTRMSNorm, got {type(rmsnorm)}"

    maxval = max(requant.max, -requant.min)

    if affine:
        new_weight = rmsnorm.weight
        new_rmsnorm = PACTIntegerRMSNorm(n_levels = requant.n_levels,
                                         eps_in = eps_in,
                                         maxval = maxval,
                                         weight = new_weight,
                                         D = D,
...
class IntegerizeRMSNormPass(SequentialPass):
    ...
    def __init__(self, symbolic_trace: callable, affine = True, D = 2**12, export_rmsnorm_node = False, **kwargs):
        ...
        passes.append(
            ReplaceSequentialPatternPass(
                pattern, symbolic_trace,
                partial(integerize_rmsnorm_fun, affine = affine, D = D, export_node = export_rmsnorm_node),
                f'_INTEGER_RMSNORM_PASS'))
...
class IntegerizeSoftmaxPass(SequentialPass):
    ...
    def __init__(self,
        ...
        passes.append(
            ReplaceSequentialPatternPass(
                pattern, symbolic_trace,
                partial(integerize_softmax_fun, mode = 'I-BERT', export_node = export_softmax_node),
                f'_INTEGER_SOFTMAX_PASS'))

        pattern = nn.Sequential(PACTITAMax())
        passes.append(
            ReplaceSequentialPatternPass(
                pattern, symbolic_trace,
                partial(integerize_softmax_fun, mode = 'ITA', D = D, export_node = export_softmax_node),
                f'_INTEGER_SOFTMAX_PASS'))
        ...
        passes.append(
            ReplaceSequentialPatternPass(
                pattern, symbolic_trace,
                partial(integerize_softmax_fun, mode = 'ITA-Partial', D = D, export_node = export_softmax_node),
                f'_INTEGER_SOFTMAX_PASS'))
...
class IntegerizeGELUPass(SequentialPass):
    ...
    def __init__(self, D = 2**14, export_gelu_node = False, symbolic_trace: callable = PACT_symbolic_trace, **kwargs):
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace,
                                         partial(integerize_gelu_fun, D = D, export_node = export_gelu_node),
                                         f'_INTEGER_GELU_PASS'))
...
class IntegerizeHardswishPass(SequentialPass):
    ...
    def __init__(self, export_hardswish_node = False, symbolic_trace: callable = PACT_symbolic_trace, **kwargs):
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace,
                                         partial(integerize_hardswish_fun, export_node = export_hardswish_node),
                                         f'_INTEGER_SILU_PASS'))
...
def integerize_pact_conv_fun(gm: fx.GraphModule, match: Match):
    ...
    assert isinstance(
        conv, (PACTConv1d, PACTConv2d)), f"integerize_pact_conv_fun got bad match - expected PACTConv, got {type(conv)}"
    if conv.bias is not None:
        print(
            f"integerize_pact_conv_fun: WARNING - FOUND CONV LAYER WITH BIAS; MAKE SURE THIS IS INTENDED!\nLAYER NAME: {conv_node.target}"
        )
...
def integerize_pact_linear_fun(gm: fx.GraphModule, match: Match):
    ...
    eps_in = extract_eps(lin_node.meta['quant'].eps_in)
    assert isinstance(lin,
                      PACTLinear), f"integerize_pact_linear_fun got bad match - expected PACTLinear, got {type(lin)}"
    ...
    new_lin = nn.Linear(in_features = lin.in_features, out_features = lin.out_features, bias = (lin.bias is not None))
...
def swap_maxpool_act_fun(gm: fx.GraphModule, match: Match):
    ...
    assert isinstance(
        maxpool,
        (nn.MaxPool2d)), f"swap_maxpool_act_fun got bad match - expected torch.nn.MaxPool2d, got {type(maxpool)}"
    assert isinstance(
        act,
        (PACTUnsignedAct, PACTAsymmetricAct
        )), f"swap_maxpool_act_fun got bad match - expected 'PACTUnsignedAct' or 'PACTAsymmetricAct', got {type(act)}"
        ...
        for act_name, act_type in [("UNSIGNED_ACT", PACTUnsignedAct), ("SIGNED_ACT", PACTAsymmetricAct)]:
            for mp_name, mp_type in [("MP2D", nn.MaxPool2d)]:
                pattern = nn.Sequential(
                    mp_type(kernel_size = 2),
                    act_type(n_levels = 256, init_clip = 'max', learn_clip = False, act_kind = 'identity'))
                passes.append(
                    ReplaceSequentialPatternPass(pattern, symbolic_trace, swap_maxpool_act_fun,
                                                 f"_SWAP_{mp_name}_{act_name}_PASS"))
...
def replace_pact_causalconv1d_padconv1d_fun(gm: fx.GraphModule, match: Match):
    ...
    new_module = nn.Sequential(
        nn.ConstantPad1d(padding = (causalconv._PACTCausalConv1d__padding, 0), value = 0),
        PACTConv1d(in_channels = causalconv.in_channels,
...
class ReplacePACTCausalConv1DPass(ReplaceSequentialPatternPass):
    ...
    def __init__(self, symbolic_trace: callable = PACT_symbolic_trace):
        ...
        super(ReplacePACTCausalConv1DPass, self).__init__(pattern, symbolic_trace,
                                                          replace_pact_causalconv1d_padconv1d_fun, name)
    ...
    def __init__(
        ...
        for act_name, act_type in [("UNSIGNED_ACT", PACTUnsignedAct), ("SIGNED_ACT", PACTAsymmetricAct)]:
            for bn_name, bn_type in [("BN1D", nn.BatchNorm1d), ("BN2D", nn.BatchNorm2d), ("BN3D", nn.BatchNorm3d)]:
                pattern = nn.Sequential(
                    bn_type(1), act_type(n_levels = 256, init_clip = 'max', learn_clip = False, act_kind = 'identity'))
                passes.append(
                    ReplaceSequentialPatternPass(pattern,
                                                 symbolic_trace,
                                                 bn_act_to_requant_fun,
                                                 f"_INTEGERIZE_{bn_name}_{act_name}_PASS",
                                                 D = D,
                                                 recenter_act = recenter_act,
                                                 cmsis_requant = cmsis_requant,
                                                 requant_node = requant_node,
                                                 skip_identity_rqs = skip_identity_rqs))
...
def conv_bn_act_to_conv_threshold_fun(gm: fx.GraphModule, match: Match, cutie_style_threshs = False):
    ...
    assert len(matched_nodes) == len(
        matched_modules), "conv_bn_act_to_conv_threshold_fun got unexpected non-'call_module' nodes!"
    conv = matched_modules[0]
    conv_node = matched_nodes[0]
    assert isinstance(
        conv,
        (nn.Conv1d, nn.Conv2d)), f"conv_bn_act_to_conv_threshold_fun called on incompatible Conv layer {type(conv)}"
    ...
    if len(matched_modules) == 2:
        ...
        assert isinstance(
            bn, (nn.BatchNorm1d,
                 nn.BatchNorm2d)), f"conv_bn_act_to_conv_threshold_fun called on incompatible BN layer {type(bn)}"
    assert isinstance(
        act, (PACTUnsignedAct,
              PACTAsymmetricAct)), f"conv_bn_act_to_conv_threshold_fun called on incompatible activation {type(act)}"
    ...
    if prev_node.meta['quant'].signed_out:  # if the position of the node in the graph is 1, then it is the first node
        ...
        bias_add = conv.weight_q.sum(dim = (1, 2, 3)) if len(conv.weight_q.shape) == 4 else conv.weight_q.sum(dim = (1,
                                                                                                                     2))
        ...
        for conv_name, conv_type in [('CONV1D', nn.Conv1d), ('CONV2D', nn.Conv2d)]:
            for act_name, act_type in [("UNSIGNED_ACT", PACTUnsignedAct), ("SIGNED_ACT", PACTAsymmetricAct)]:
                for bn_name, bn_type in [("BN1D", nn.BatchNorm1d), ("BN2D", nn.BatchNorm2d)]:
                    pattern = nn.Sequential(
                        conv_type(1, 1, 1), bn_type(1),
                        act_type(n_levels = 256, init_clip = 'max', learn_clip = False, act_kind = 'identity'))
                    passes.append(
                        ReplaceSequentialPatternPass(pattern, PACT_symbolic_trace, conv_bn_act_to_conv_threshold_fun,
                                                     f"_{bn_name}_{act_name}_TO_THRESHOLD_PASS"))
            ...
            passes.append(
                ReplaceSequentialPatternPass(pattern, PACT_symbolic_trace, conv_bn_act_to_conv_threshold_fun,
                                             f"_{act_name}_TO_THRESHOLD_PASS"))
...
def embedding_integerize_fun(gm: fx.GraphModule, match: Match, **kwargs):
    ...
    new_embedding = PACTIntegerEmbedding(n_levels = n_levels,
                                         weight = bias,
                                         eps_in = eps_in,
                                         eps_adder = eps_adder,
                                         maxval = maxval,
...
class IntegerizeEmbeddingsPass(SequentialPass):
    ...
    def __init__(self, symbolic_trace: callable = PACT_symbolic_trace, **kwargs):
        passes = []
        pattern = nn.Sequential(
            PACTEmbedding(torch.Tensor((1.,)), init_clip = 'max', learn_clip = False, act_kind = 'identity'))
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace, partial(embedding_integerize_fun, **kwargs),
                                         f'_INTEGERIZE_EMBEDDINGS_PASS'))
    ...
    def fix_conv_channels(self, gm: fx.GraphModule, node: fx.Node, force_out_channels: int, bw_out: int = 0):
        if node.op == 'call_module' and node not in self.visited_nodes:
            ...
            if isinstance(module, (PACTConv1d, PACTConv2d)):
                ...
                assert module.groups in [
                    1, module.in_channels
                ], f"fix_conv_channels: Unsupported groups config for conv {module}; {module.groups} groups not supported"
                ...
                new_weights = torch.zeros(
                    tuple([new_out_ch, (new_in_ch // new_groups)] + [k for k in module.kernel_size])).type_as(
                        module.weight.data)
                ...
                print(
                    f"Adjusting Conv {node.target}'s channels: {module.in_channels}/{module.out_channels} ==> {new_in_ch}/{new_out_ch}"
                )
            ...
            # network, perform output channel alignment
                ...
                for inp in node.all_input_nodes:
                    self.fix_conv_channels(gm, inp, force_out_channels,
                                           np.ceil(np.log2(node.meta['quant'].n_levels_out)))
            else:
                self.visited_nodes.add(node)
                for inp in node.all_input_nodes:
                    self.fix_conv_channels(gm, inp, force_out_channels)
    ...
    def run_pass(self, gm: fx.GraphModule):
        ...
        for in_node in input_nodes:
            ...
            user_conv_modules = [
                module_of_node(u)
                for u in users
                if u.op == 'call_module' and isinstance(module_of_node(u), (nn.Conv1d, nn.Conv2d, nn.Conv3d))
            ]
            if len(users) != len(user_conv_modules):
                print(
                    f"SignedToUnsignedInputPass will likely create bogus - input is used by non-module or non-Conv nodes!"
                )
...
class IntegerizeBNPACTHardActsPass(SequentialPass):
    ...
    def bn_hardact_qact_to_requant_fun(gm: fx.GraphModule, match: Match, D1 = 2**19, D2 = 2**10):
        ...
        ha_requant = HardActRequantShift(gamma_h,
                                         beta_h,
                                         three = three,
                                         six = six,
                                         D1 = D1,
                                         D2 = D2,
                                         hsigmoid = is_hardsigmoid,
                                         c_lo = c_lo,
                                         c_hi = c_hi,
                                         eps_half = eps_half)
        ...
        for act_name, act_type in [("HARDSIGMOID", PACTHardsigmoid), ("HARDSWISH", PACTHardswish)]:
            for bn_name, bn_type in [("BN1D", nn.BatchNorm1d), ("BN2D", nn.BatchNorm2d), ("BN3D", nn.BatchNorm3d)]:
                ...
                passes.append(
                    ReplaceSequentialPatternPass(pattern,
                                                 symbolic_trace,
                                                 self.bn_hardact_qact_to_requant_fun,
                                                 f"_INTEGERIZE_{bn_name}_{act_name}_PASS",
                                                 D1 = D1,
                                                 D2 = D2))
...
class IntegerizePACTNetPass(SequentialPass):
    ...
    def __init__(self,
                 ...
                 signed_in: bool = True,
                 D: float = 2**24,
                 enable_add_first = False,
                 requant_node = False,
                 n_levels_in: int = 256,
                 fix_channel_numbers = False,
                 convert_input_to_unsigned: bool = False,
                 D1: float = 2**18,
                 D2: float = 2**12,
                 ternarize: bool = False,
                 word_align_channels: bool = False,
                 export_layernorm_node = False,
                 export_softmax_node = False,
                 export_gelu_node = False,
                 export_div_node = False,
                 export_rmsnorm_node = False,
                 export_hardswish_node = False,
                 skip_identity_rqs = True,
                 symbolic_trace = PACT_symbolic_trace,
                 verbose = False):
        ...
        passes.append(
            IntegerizeRMSNormPass(D = D, symbolic_trace = symbolic_trace, export_rmsnorm_node = export_rmsnorm_node))
        ...
        passes.append(
            IntegerizeSoftmaxPass(D = D, symbolic_trace = symbolic_trace, export_softmax_node = export_softmax_node))
        passes.append(
            IntegerizeLayerNormPass(D = D,
                                    symbolic_trace = symbolic_trace,
                                    export_layernorm_node = export_layernorm_node))

        passes.append(
            IntegerizeHardswishPass(symbolic_trace = symbolic_trace, export_hardswish_node = export_hardswish_node))

        passes.append(IntegerizeGELUPass(D = D, symbolic_trace = symbolic_trace, export_gelu_node = export_gelu_node))
        passes.append(
            IntegerizeBNActPass(D,
                                enable_add_first,
                                symbolic_trace = symbolic_trace,
                                requant_node = requant_node,
                                skip_identity_rqs = skip_identity_rqs))
        passes.append(
            IntegerizeEmbeddingsPass(cmsis_requant = enable_add_first,
                                     symbolic_trace = symbolic_trace,
                                     requant_node = requant_node))
