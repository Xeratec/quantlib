...
from quantlib.algorithms.pact.pact_ops import PACTEmbedding, PACTGELU, PACTHardswish, PACTITAMax, PACTITAPartialMax, PACTIntegerAdd, PACTLayerNorm, PACTRMSNorm, PACTSoftmax
...
class ApproximateSoftmaxPass(SequentialPass):
    ...
    def __init__(self,
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace, partial(replSoftmax, mode = mode, **kwargs),
...
class ApproximateGELUPass(SequentialPass):
    ...
    def __init__(self,
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace, lambda x, y: PACTGELU(), f'_APPROXIMATE_GELU_PASS'))
...
class ApproximateSiLUPass(SequentialPass):
    ...
    def __init__(self,
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace, lambda x, y: PACTHardswish(eps_s = 1 / n_levels),
                                         f'_APPROXIMATE_SILU_PASS'))
...
def layernorm_replacement_fun(gm: fx.GraphModule, match: Match, *args, **kwargs):
    ...
    assert isinstance(
        layernorm, nn.LayerNorm), f"layernorm_replacement_fun got bad match - expected LayerNorm, got {type(layernorm)}"

    weight = layernorm._parameters['weight'].clone() if layernorm._parameters['weight'] is not None else torch.Tensor(
        (1.,))
...
class CanonicalizeLayerNormPass(SequentialPass):
    ...
    def __init__(self,
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace, partial(layernorm_replacement_fun, *args, **kwargs),
                                         f'_CANONICALIZE_LAYERNORM_PASS'))
...
def rmsnorm_replacement_fun(custom_module, gm: fx.GraphModule, match: Match, *args, **kwargs):
    ...
    assert isinstance(
        rmsnorm,
        type(custom_module)), f"rmsnorm_replacement_fun got bad match - expected LayerNorm, got {type(rmsnorm)}"
    ...
    def __init__(self, symbolic_trace: callable, custom_module, *args, **kwargs):
        ...
        passes.append(
            ReplaceSequentialPatternPass(pattern, symbolic_trace,
                                         partial(rmsnorm_replacement_fun, custom_module, *args, **kwargs),
                                         f'_CANONICALIZE_RMSNORM_PASS'))
...
class ProtoPACTEmbedding(torch.nn.Module):
    ...
    def __init__(self, weights: torch.Tensor = torch.Tensor((1.,))):
        ...
        self.adder = PACTIntegerAdd(n_levels = 256,
                                    num_args = 2,
                                    act_kind = 'identity',
                                    init_clip = 'max',
                                    learn_clip = True)
...
class CanonicalizeEmbeddingsPass(SequentialPass):
    ...
    def __init__(self, n_levels: int = 256, **kwargs):
        ...
        passes.append(
            ReplaceSingleInputPatternPass(pattern, PACT_symbolic_trace_inclusive,
                                          partial(embedding_replacement_fun, n_levels = n_levels),
                                          f'_CANONICALIZE_EMBEDDING_PASS'))
