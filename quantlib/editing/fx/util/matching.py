...
class SequentialMatcher:
    ...
    def __init__(self, pattern: callable, trace: callable = fx.symbolic_trace):
        ...
        for n in p.graph.nodes:
            assert (n.op == 'placeholder' and len(n.users) == 1) or (n.op == 'output' and len(
                n.all_input_nodes) == 1) or (len(n.all_input_nodes) == 1
                                             and len(n.users) == 1), "Only sequential patterns are supported!"
    ...
    def _match_nodes(self, pn: fx.Node, gn: fx.Node, last_active_node: bool = False, nodes_map: dict = None):
        ...
        def attributes_are_equal(pn: fx.Node, gn: fx.Node) -> bool:
            ...
            if pn.op != "call_module":
                ...
                return pn.op == gn.op and isinstance(self.searched_modules[gn.target],
    ...
    def match_graph(self, gm: fx.GraphModule):
        ...
        def match_overlaps_with_previous(match):
            return any(
                n in matched_nodes and n.op not in ("placeholder", "output") and k.op not in ("placeholder", "output")
