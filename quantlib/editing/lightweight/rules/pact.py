...
from quantlib.algorithms.pact.pact_ops import PACTAsymmetricAct, PACTCausalConv1d, PACTConv1d, PACTConv2d, PACTHardsigmoid, PACTHardswish, PACTLinear, PACTUnsignedAct
from quantlib.algorithms.generic import CausalConv1d
from .rules import LightweightRule
from .filters import Filter


def replace_pact_conv_linear(module: Union[nn.Conv1d, nn.Conv2d, nn.Linear, CausalConv1d], **kwargs):
...
def replace_pact_act(module: nn.Module, signed: bool = False, **kwargs):
...
def replace_pact_hard_act(module: nn.Module, hact_kwargs: dict, quant_act_kwargs: dict):
    if isinstance(module, nn.Hardsigmoid):
        layers = [PACTHardsigmoid(**hact_kwargs), PACTUnsignedAct(**quant_act_kwargs)]
    elif isinstance(module, nn.Hardswish):
        layers = [PACTHardswish(**hact_kwargs), PACTAsymmetricAct(**quant_act_kwargs)]
    return nn.Sequential(*layers)


def pact_quant_hard_act(module: nn.Module, quant_act_kwargs: dict):
    if isinstance(module, nn.Hardsigmoid):
        layers = [nn.Hardsigmoid(), PACTUnsignedAct(**quant_act_kwargs)]
    elif isinstance(module, nn.Hardswish):
        layers = [nn.Hardswish(), PACTAsymmetricAct(**quant_act_kwargs)]
    return nn.Sequential(*layers)


def quantize_pool_pact(module: nn.Module, signed: bool, **quant_act_kwargs):
    if signed:
...
class ReplaceActPACTRule(LightweightRule):
    ...
    def __init__(self, filter_: Filter, signed: bool = False, **kwargs):
...
class ReplaceHardActPACTRule(LightweightRule):
    ...
    def __init__(self,
        ...
        if use_pact_hact:
            replacement_fun = partial(replace_pact_hard_act,
                                      hact_kwargs = hact_kwargs,
                                      quant_act_kwargs = quant_act_kwargs)
        else:
