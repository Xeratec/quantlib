...
def replace_conv2d_inqconv2d(module: torch.nn.Module, num_levels: int, quant_init_method: Union[str, None],
...
class ReplaceConv2dINQConv2dRule(LightweightRule):
    ...
    def __init__(self,
        ...
        replacement_fun = partial(
            replace_conv2d_inqconv2d,
            num_levels = num_levels,
            quant_init_method = quant_init_method,
            quant_strategy = quant_strategy
        )  # currying returns a function with the correct signature ``Callable[[torch.nn.Module], torch.nn.Module]``
